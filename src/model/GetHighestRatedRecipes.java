...
public class GetHighestRatedRecipes {
    private final int topN;

    public GetHighestRatedRecipes(int topN) {
        this.topN = topN;
    }
    ...
    public ArrayList<Recipe> getRecipes() {
        try {
            ...
            String selectRecipes = "SELECT * FROM recipe ORDER BY rating DESC LIMIT " + topN;
            ResultSet rs;
    ...
    public static void main(String[] args) {
        ...
        if (args.length == 1) {
            GetHighestRatedRecipes search = new GetHighestRatedRecipes(Integer.parseInt(args[0]));
