...
public class GetMostRecentRecipes {
    private final int topN;

    public GetMostRecentRecipes(int topN) {
        this.topN = topN;
    }
    ...
    public ArrayList<Recipe> getRecipes() {
        try {
            ...
            String selectRecipes = "SELECT * FROM recipe ORDER BY date_made DESC, time_made DESC LIMIT " + topN;
            ResultSet rs;
    ...
    public static void main(String[] args) {
        ...
        if (args.length == 1) {
            GetMostRecentRecipes search = new GetMostRecentRecipes(Integer.parseInt(args[0]));
